<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Calls Table</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet"
    integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM" crossorigin="anonymous">
  <link rel="stylesheet" href="css/style.css">
</head>
<body>
  <div class="container">
  <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
    <a href="/" class="d-flex align-items-center my-2 my-lg-0 me-lg-auto text-decoration-none"> my logo
    </a>

    <ul class="nav col-12 col-lg-auto my-2 justify-content-center my-md-0 text-small">
      
      <li>
        <a href="workorder.html" class="nav-link">
          Add New Call
        </a>
      </li>
      <li>
        <a href="viewcalltable.html" class="nav-link">
          View Calls
        </a>
      </li>
      <li>
        <a href="engtable.html" class="nav-link">
          Engineers
        </a>
      </li>
      <li>
        <a href="index.html" class="nav-link">
          Log Out
        </a>
      </li>
      
    </ul>
  </div>
  <table class="table table-bordered">
    <thead>
      <tr>
        <th scope="col">SNo.</th>
        <th scope="col">Employee Name</th>
        <th scope="col">Email to notify</th>
        <th scope="col">Department</th>
        <th scope="col">Designation</th>
        <th scope="col">Engineer Name</th>
        <th scope="col">Status</th>
        <th scope="col">Level</th>
        <th scope="col">Subject</th>
        <th scope="col">Description</th>
        <th scope="col">Priority</th>
        <th scope="col">Action</th>
      </tr>
    </thead>
    <tbody>
  
      <tr>
        <th scope="row">1</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>


      </tr>
      <tr>
        <th scope="row">2</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>

      </tr>
      <tr>
        <th scope="row">3</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>

      </tr>
      <tr>
        <th scope="row">4</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>

      </tr>
      <tr>
        <th scope="row">5</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>
      </tr>
      <tr>
        <th scope="row">6</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>
      </tr>
      <tr>
        <th scope="row">7</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>
      </tr>
      <tr>
        <th scope="row">8</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>

      </tr>
      <tr>
        <th scope="row">9</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>

      </tr>
      <tr>
        <th scope="row">10</th>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td></td>
        <td>
          <button class="btn btn-info"><a class="edit_btn">Edit</a></button>
          <button class="btn btn-danger"><a class="delete_btn" href="">Delete</a></button>
        </td>

      </tr>
    </tbody>
  </table>
  </div>


  <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"
    integrity="sha384-geWF76RCwLtnZ8qwWowPQNguL3RmwHVBC9FhGdlKrxdiJJigb/j/68SIy3Te4Bkz"
    crossorigin="anonymous"></script>

</body>

</html>

<?php
include"config.php";
$sql="SELECT FROM users";
$result = $conn->query($sql);
?>